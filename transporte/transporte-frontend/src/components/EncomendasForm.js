...
const EncomendasForm = ({ rotaId }) => {
	...
	const imprimirEtiqueta = (encomenda) => {
		...
		janela.document.write(`
			...
			<head>
				...
				<style>
					...
					.label { border: 1px dashed #333; padding: 10px; width: 250px; }
				</style>
			</head>
			<body>
				<div class="label">
					<h3>Encomenda</h3>
					<p><strong>Ref.:</strong> ${encomenda.ref_bagagem}</p>
					<p><strong>Peso:</strong> ${encomenda.peso_kg} KG</p>
					<p><strong>Remetente:</strong> ${encomenda.nome_remetente}</p>
					<p><strong>Tel:</strong> ${encomenda.telefone_remetente}</p>
					<p><strong>Receptor:</strong> ${encomenda.nome_receptor}</p>
					<p><strong>Tel:</strong> ${encomenda.telefone_receptor}</p>
				</div>
    ...
    const salvarEncomendas = () => {
        ...
        if (validas.length === 0) {
            alert("Preencha pelo menos uma encomenda com os dados obrigatórios.");
            return;
        }
        ...
        Promise.all(
            ...
            .catch((err) => {
                ...
                if (err.response?.data) {
                    ...
                    alert("Erro ao salvar encomendas. Verifique a conexão.");
                }
            });
    };
    ...
    return (
        <div>
            <h3>📦 Encomendas Não Acompanhadas</h3>
            {encomendas.map((enc, index) => (
                <div key={index} style={{ border: "1px solid #ccc", padding: 10, marginBottom: 10 }}>
					...
					<button onClick={() => imprimirEtiqueta(enc)}>🖨️ Etiqueta</button>
                </div>
            ))}
            <button onClick={adicionarLinha}>+ Adicionar Encomenda</button>
            <button onClick={salvarEncomendas}>💾 Salvar Encomendas</button>
        </div>
    );
};
